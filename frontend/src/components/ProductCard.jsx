...
const cardCss = `
  ...
  .kpc-badge-stock {
    background: rgba(255,237,180,0.95);
    color: #92610a;
  ...
  .kpc-btn-active {
    background: #1a4d1a;
    color: #fff;
    box-shadow: 0 3px 14px rgba(26,77,26,0.28);
  }
  ...
  .kpc-btn-added {
    background: #e8f5e9;
    color: #2d7d2d;
    border: 1.5px solid rgba(45,125,45,0.2);
    cursor: default;
  }
  ...
  .kpc-check {
    ...
    animation: kpc-check 0.35s cubic-bezier(0.34,1.56,0.64,1);
  }

  /* Out-of-stock overlay tint */
...
export default function ProductCard({ product }) {
  ...
  return (
    <>
      <style>{cardCss}</style>
      <Link to={`/products/${product._id}`} className="kpc-card">
        {/* Image */}
        <div className={`kpc-img-wrap${!product.stock ? " oos" : ""}`}>
          ...
          {/* Badges */}
          {hasDiscount && (
            <span className="kpc-badge kpc-badge-discount">−{discountPct}%</span>
          )}
        ...
        <div className="kpc-body">
          <div className="kpc-top">
            ...
            <div className="kpc-price-wrap">
              ...
              {hasDiscount && (
                <div className="kpc-price-orig">{currency(product.price)}</div>
              )}
            </div>
          </div>
