...
const cardCss = `
  ...
  .kpc-btn-wrap {
    ...
    border-radius: 14px; margin-top: auto;
  }
  .kpc-btn {
...
export default function ProductCard({ product }) {
  // NOTE: useAuth() reads from AuthContext. If `user` is null here despite being
  // logged in, it means AuthContext.loadUser() failed to set state — check the
  // AuthContext response unwrapping and the /auth/me endpoint.
  const { user } = useAuth();
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      // addItem already sets error in CartContext; nothing more needed here
    }
  };
